    ...
    */
    ...
    function show_navi()
    {
        if ($_REQUEST['action'] == 'stats') {
            ...
            <a href="?page=<?php echo $_REQUEST['page']; ?>"
               class="button-primary"><?php _e('View options', 'wpnoexternallinks'); ?></a>
        <?php } else { ?>
            <a href="?page=<?php echo $_REQUEST['page']; ?>&action=stats"
               class="button-primary"><?php _e('View Stats', 'wpnoexternallinks'); ?></a>
        <?php } ?>
        <a href="http://jehy.ru/articles/2008/10/05/wordpress-plugin-no-external-links/"
           class="button-primary"><?php _e('Feedback', 'wpnoexternallinks'); ?></a>
        <a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=EE8RM4N7BSNZ6"
           class="button-primary"><?php _e('Donate', 'wpnoexternallinks'); ?></a>
    <?php
    }
    ...
    function view_stats()
    {
        ...
        if (!$this->options['stats']) {
            ...
            ?>
            <input type="text" name="date1" value="<?php echo $date1; ?>"><?php _e(' to ', 'wpnoexternallinks'); ?>
            <input type="text" name="date2" value="<?php echo $date2; ?>"><input type="submit"
                                                                                 value="<?php _e('View', 'wpnoexternallinks'); ?>"
    ...
    function option_page()
    {
        ...
        <p>
            <?php echo __('If you need to mask links in posts`s custom field, take a look at', 'wpnoexternallinks') . ' <a href="http://jehy.ru/articles/2015/03/06/masking-links-in-custom-fields-with-wp-noexternallinks/">' . __('this article.', 'wpnoexternallinks') . '</a>.'; ?>
        </p>
        <form method="post" action="">
            <?php wp_nonce_field('update-options');
            $this->show_navi();?>
            <br>
            <?php echo '<h2>' . __('Global links masking settings', 'wpnoexternallinks') . '</h2>' . '(' . __('You can also disable plugin on per-post basis', 'wpnoexternallinks') . ')'; ?>
            <br><br>
            <?php
            ...
            ?><input type="submit" name="submit" value="<?php _e('Save Changes', 'wpnoexternallinks') ?>"
                     class="button-primary"/>
        </form>
    <?php
    }
